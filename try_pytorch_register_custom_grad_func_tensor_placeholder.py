...
class TorchTensor:
    """
    From https://github.com/FMInference/FlexGen/blob/main/flexgen/pytorch_backend.py
...
class LegendrePolynomial3(torch.autograd.Function):
    ...
    def forward(ctx, inputs):
        ...
        inputs_placeholder_to_save = TorchTensor(
            inputs_to_save.shape,
            inputs_to_save.dtype,
            inputs_to_save,
            torch.device("cpu"),
        )
        # ctx.save_for_backward(inputs_to_save)
        ctx.backward_cache = inputs_placeholder_to_save
    ...
    def backward(ctx, grad_output):
        ...
        """
        # (inputs_ref,) = ctx.saved_tensors
        inputs_placeholders = ctx.backward_cache
        inputs = inputs_placeholders.data
